...
- **MCP server 装载（per-server 三重门禁）** — `config.mcpServers` 每项支持 `enabled` / `roles` / `allowedTools`，对所有 server 通用，不为某个 server 单开顶层块。`type: 'stdio'`（默认）走子进程；`type: 'builtin'` 走 `src/mcp-builtin/<name>.ts` 同进程实现，由 `BUILTIN_FACTORIES` 注册。Interrogator 永不挂 MCP（硬约束，纯对话阶段不应主动探索）。当前默认：Playwright（stdio，Generator + Evaluator）开；computer-use（builtin、macOS-only、cliclick 依赖）默认 disabled，opt-in 后仅给 Evaluator，精确白名单 `screenshot/left_click/type/key`。详见 SPEC §八 / §10.7。
