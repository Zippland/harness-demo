...
**MCP 挂载机制**：每个 server 有两层独立门禁 —— `enabled`（启停）/ `allowedTools`（在 agent allowedTools 列表里展开为通配或精确白名单）。

设计原则是 **capability 通用 + policy 走 prompt**：tool 不分 role 限制，所有非 Interrogator 角色装载同一组 MCP，行为差异由系统提示词区分（Generator 实现、Evaluator 验证）。详见 §10.8。

唯一硬约束：**Interrogator 永远不挂任何 MCP**，编码在 `src/config.ts` 的 `isServerEnabledForRole` 里，不通过 per-server 配置控制。理由：Interrogator 的 prompt 强约束"只反问、不主动探索"，给它 tool 等于打破这个约束的诱惑——这是边界例外，不能推广到 Generator vs Evaluator。

`config.default.json` 内置两个 server，**全部默认启用、对所有非 Interrogator 角色可见**：

| Server | 形态 | 默认 enabled | 工具白名单 |
|---|---|---|---|
| `playwright` (`@playwright/mcp`) | stdio | ✓ | `mcp__playwright__*` 通配 |
| `computer-use` (in-process) | builtin | ✓ | 精确白名单（`screenshot/left_click/type/key`） |

`computer-use` 详见 §10.7。关闭方式：`.harness/config.json` 里写 `{"mcpServers": {"computer-use": {"enabled": false}}}`。
...
2. **架构对齐**：C 复用 SDK 的 `mcpServers` 装载点和 §8 per-server 双重门禁（`enabled / allowedTools`），不引入并行机制。A 也对齐这一点，但本身的稳定性消化不了。
...
### 10.8 为什么 MCP 装载没有 per-role 隔离（capability vs policy）

设计原则：**capability 走 tool 装载、policy 走 prompt**。这两层不应在同一个轴上耦合。

具体到 MCP：所有非 Interrogator 角色装载同一组 MCP server（capability 通用），Generator 与 Evaluator 的行为差异完全由系统提示词区分（Generator 实现、Evaluator 验证）。`McpServerSpec` 不暴露 `roles` 字段——曾经短暂存在过，删除了。

**为什么不能用 per-role tool 隔离当安全墙**：

- **不创造真实安全保证**：Generator 已经有 `Bash`，能 `rm -rf /`、能 `curl` 任意端点、能改 `~/.ssh/`。在 Bash 已存在的前提下，computer use / Playwright 不增加可达攻击面的量级——只增加触达方式。
- **副作用大小取决于 tool 本身，不取决于哪个 role 调**：一个失控的 `type "rm -rf /\n"` 在 Generator 还是 Evaluator session 里发出，结果一样。
- **per-role 隔离是分工 hint，不是约束**：用 prompt 表达分工天然且可读；用 tool 表达则把"分工"和"权限"混在一根轴上，为未来加新 role 增加 N×M 的权限矩阵维护成本。
- **历史教训**：本文档曾在 §10.7 写过"评估副作用 < 创造副作用，所以 computer-use 仅 Evaluator"——这条论据是错的，已经撤掉。现在留作反例：tool 隔离的诱惑常常来自把"看上去更安全"误当成"实际更安全"。

**Interrogator 是边界例外**：它的 prompt 强约束"只反问、永不 propose"，给它 tool（哪怕只读）会成为打破约束的诱惑——观察过 Interrogator 拿到 `Read` 后偷偷去 `Glob` 整个项目结构再"反问"的退化案例。所以 Interrogator 不挂 MCP 是硬编码的、不可配置的——这是 capability/policy 解耦原则在边界条件上的失败处理，不是普适规则。Generator vs Evaluator 不在这个边界上。

**与 Playwright 的对齐验证**：Playwright 从一开始就是 G + E 都挂——没人提议过"Playwright 仅 Evaluator"。computer-use 一旦从"特殊"还原到"通用 capability"，就该和 Playwright 同等对待。
