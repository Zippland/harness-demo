...
1.2 一个例子：Anthropic 的 Agent Skills
...
2.3 用控制论拆解 Anthropic 的 Harness
...
2.3.2 传感器：Playwright MCP + 四维评分体系
...
2.3.3 控制器：Sprint Contract Criterion
...
2.3.5 反馈回路的进化：从 Opus 4.5 到 Opus 4.6
...
2.5 Context 和 Harness 的分界线

现在可以把 Context 和 Harness 的分界线理清一下。
...
3.3 Anthropic（2025 年 11 月）：长时运行 Agent 的 Harness 设计
...
3.4 Superpowers：开源社区的 Harness 实践标杆

如果前面的案例都是大公司内部实践，那么 Jesse Vincent 的开源项目 Superpowers 则代表了另一维度：社区用脚投票验证的 Harness Engineering。这个项目在 5 个月内从 0 增长到超过 121K GitHub Stars，是 Agent 编程领域增长最快的开源项目之一。

Superpowers 本质上是一套给编码 Agent 用的"技能库" —— 一组可组合的 Skill 文件，强制 Agent 遵循特定的工作流。它不是一个框架或代码库，而是一套用自然语言写成的行为约束协议，支持 Claude Code、Cursor、Codex、Gemini CLI 等多个平台。

用控制论视角拆解它的架构：

1. 被控对象：上下文污染和目标偏移。 
Agent 在长时间任务中最常见的失败模式是：上下文窗口累积越来越多无关信息，导致后期任务偏离原始规格。Superpowers 的核心对策是上下文隔离 —— 每个任务派生一个全新的 Subagent，不继承主会话的历史上下文，由 Controller 精确注入它需要的信息。这等于每个执行器都从干净状态启动。

2. 参考信号：冻结的 Brainstorming → Writing Plans → Execution 链。 
  Superpowers 强制执行一个严格的三阶段前置流程，每个阶段之间有硬性关卡：
Brainstorming（探索意图）
    ↓ 用户批准 spec 才能进入下一步
Writing Plans（生成计划）
    ↓ 计划细化到 2-5 分钟一步，每步包含完整代码、文件路径、预期输出
Subagent Driven Development（执行）
    ↓ 每个任务派生独立 Subagent，两次审查后才能进入下一步
  
  关键的 HARD GATE：如果 Agent 没有完成 Brainstorming 就试图写代码，系统会直接阻断。Jesse Vincent 的验证方法是给 Agent 输入"Let's make a react todo list"：如果它直接开始编程，说明 Harness 失败了；如果它启动 brainstorming 流程，说明反馈回路生效了。

3. 传感器：两阶段独立审查。 每个 Subagent 完成任务后，必须经过两个独立的审查 Agent：
  1. Spec Compliance Reviewer——先检查"做的是不是对的事"：代码是否完全符合 spec，没有多做也没有少做
  2. Code Quality Reviewer——再检查"做得好不好"：代码质量、测试覆盖、设计模式
  
  顺序不可置换 —— 先保证正确性，再保证质量。如果任何一个审查发现问题，Implementer Subagent 修复后必须重新接受同一个审查者的复查，直到通过。

4. 执行器：模型分级的成本控制。 Superpowers 指示 Agent 对每个任务选择最廉价的足以胜任的模型。当 spec 写得足够细致时（每步 2-5 分钟、完整代码、确切文件路径），大多数实现任务可以交给 Claude Haiku 这样的轻量级模型；架构、设计、审查任务用最强模型。这是控制系统中经典的资源分配策略——把有限的计算预算集中在系统最需要判断力的节点。

最独特的设计：用说服原则驯设反馈回路。 Jesse Vincent 在 Superpowers 中自觉地运用了行为心理学家 Robert Cialdini 的说服原则来增强 Agent 的规则遵循：

- 权威原则："Skills are mandatory when they exist" —— 强制模型去做
- 承诺原则：要求 Agent 在每次使用 Skill 前公开宣布（"I'm using the writing-plans skill"），一旦宣布就更难反悔
- 稀缺原则：压力测试中用"生产环境宕机，每分钟损失 $5000"来试探 Agent 是否会跳过流程

更有意思的是，他发明了一套 "Skill 的 TDD" —— 用 RED/GREEN/REFACTOR 循环来测试和强化 Skill 本身：
RED:   创建压力情境，Subagent 在压力下跳过 Skill → 失败
GREEN: 强化 Skill 指令，重新测试 → Subagent 遵守 Skill
REFACTOR: 优化指令表达，减少冗余

换句话说，Skill 文件就是反馈回路的实现，而 TDD for Skills 是对反馈回路本身做反馈控制——元控制。这和 OpenAI 的"垃圾回收 Agent"、Anthropic 的"每个组件编码一个假设"原则异曲同工：都是在用系统性的方法迭代 Harness 本身。

从社区反馈看，最常被引用的一个细节是：如果 Agent 在没写测试的情况下先写了代码，Superpowers 会删除那些代码，强制从测试开始重做。 这不是警告，是硬约束。多个用户报告在生产应用上达到 85-95% 的测试覆盖率，而且是 Agent 自动完成的，不需要人工写测试。

Superpowers 的爆发式增长本身就是对 Harness Engineering 最大的行业佐证：开发者最稀缺的不是更强的模型，而是一套能让任何模型稳定运行的反馈约束体系。



3.5 四个维度，一个结论

四个团队，四个不同规模，四个不同场景，各自触及了 Harness Engineering 的一个维度：
...
5. Andrej Karpathy, Twitter/X, 2025 年 6 月 25 日. https://x.com/karpathy/status/1937902205765607626
